...
/**
 * (stolen from okhttp DiskLruCache)
 * ********** Crashy: MODIFIED TO ALLOW HOOKING INTO WHEN DELETION HAPPENS, SO WE CAN SAVE ENTRIES TO CLOUD ***********
 *
...
class CrashyDiskLruCache(
    ...
    private val onEvict: (Snapshot, DiskRemovalCause) -> Unit
) : Closeable, Flushable {
    ...
    private val cleanupTask = object : Task("$okHttpName Cache") {
        override fun runOnce(): Long {
            synchronized(this@CrashyDiskLruCache) {
    ...
    private fun newJournalWriter(): BufferedSink {
        ...
        val faultHidingSink = FaultHidingSink(fileSink) {
            this@CrashyDiskLruCache.assertThreadHoldsLock()
    ...
    internal fun completeEdit(editor: Editor, success: Boolean) {
        ...
        entry.currentEditor = null
        if (entry.zombieCause != null) {
            removeEntry(entry, entry.zombieCause!!)
            return
        }
    ...
    fun remove(key: String): Boolean {
        ...
        val removed = removeEntry(entry, DiskRemovalCause.Requested)
        if (removed && size <= maxSize) mostRecentTrimFailed = false
        return removed
    }

    @Throws(IOException::class)
    private fun removeEntry(entry: Entry, cause: DiskRemovalCause): Boolean {
        ...
        if (!civilizedFileSystem) {
            ...
            if (entry.lockingSourceCount > 0 || entry.currentEditor != null) {
                entry.zombieCause = cause
                return true
            }
        }

        entry.currentEditor?.detach(cause) // Prevent the edit from completing normally.

        // ***** CRASHY: ******* get the snapshot to save the entry to cloud before deleting it and submit it to onEvict()
        val snapshot = entry.snapshot() ?: run {
            CrashyLogger.error("Could not get snapshot of entry '${entry.key}' before deleting it, it will not be removed from cache.")
            return false
        }

        onEvict(snapshot, cause)
    ...
    override fun close() {
        ...
        for (entry in lruEntries.values.toTypedArray()) {
            if (entry.currentEditor != null) {
                entry.currentEditor?.detach(DiskRemovalCause.InvalidState) // Prevent the edit from completing normally.
            }
        }
    ...
    private fun removeOldestEntry(): Boolean {
        for (toEvict in lruEntries.values) {
            if (!toEvict.zombie) {
                removeEntry(toEvict, DiskRemovalCause.Size)
                return true
            }
        }
        return false
    ...
    fun evictAll() {
        ...
        // Copying for concurrent iteration.
        for (entry in lruEntries.values.toTypedArray()) {
            removeEntry(entry, DiskRemovalCause.Requested)
        }
        mostRecentTrimFailed = false
    }

    private fun validateKey(key: String) {
        // ***** CRASHY: ******* I don't see the need for this. This makes it annoying to store minecraft versions as keys.
//        require(LEGAL_KEY_PATTERN.matches(key)) { "keys must match regex [a-z0-9_-]{1,120}: \"$key\"" }
    ...
    fun snapshots(): MutableIterator<Snapshot> {
        ...
        return object : MutableIterator<Snapshot> {
            ...
            override fun remove() {
                ...
                try {
                    this@CrashyDiskLruCache.remove(removeSnapshot.key())
                } catch (_: IOException) {
    ...
    inner class Snapshot internal constructor(
         ...
         */
        @Throws(IOException::class)
        fun edit(): Editor? = this@CrashyDiskLruCache.edit(key, sequenceNumber)
    ...
    inner class Editor internal constructor(internal val entry: Entry) {
         ...
         */
        internal fun detach(cause: DiskRemovalCause) {
            if (entry.currentEditor == this) {
                if (civilizedFileSystem) {
                    completeEdit(this, false) // Delete it now.
                } else {
                    entry.zombieCause = cause // We can't delete it until the current edit completes.
                }
            }
        }
        ...
        fun newSink(index: Int): Sink {
            synchronized(this@CrashyDiskLruCache) {
                ...
                return FaultHidingSink(sink) {
                    synchronized(this@CrashyDiskLruCache) {
                        detach(DiskRemovalCause.InvalidState)
                    }
                }
            }
        }
    ...
    internal inner class Entry internal constructor(
        ...
        /** ****CRASHY*****: making an entry into a 'zombie' is a way of delaying deleting it until the file is freed.
         *  Because in our implementation we track the removal cause of an entry, we need to know why an entry was turned into a zombie**/
        internal var zombieCause: DiskRemovalCause? = null

        /** True if this entry must be deleted when the current edit or read completes. */
        val zombie get() = zombieCause != null
        ...
        internal fun snapshot(): Snapshot? {
            ...
            try {
                ...
                // size.)
                try {
                    removeEntry(this, DiskRemovalCause.Size)
                } catch (_: IOException) {
                }
                return null
            }
            ...
            return object : ForwardingSource(fileSource) {
                ...
                override fun close() {
                    ...
                    if (!closed) {
                        ...
                        synchronized(this@CrashyDiskLruCache) {
                            lockingSourceCount--
                            if (lockingSourceCount == 0 && zombieCause != null) {
                                removeEntry(this@Entry, zombieCause!!)
                            }
                        }
                    }
                }
    ...
    enum class DiskRemovalCause {
        Size, Requested, InvalidState
    }
}
