...
exports.testFabricCrashReportEnrich = void 0;
...
function testFabricCrashReportEnrich(enriched) {
    if (enriched.mods === undefined)
        throw new Error("Unexpected");
    expect(enriched.mods.length).toEqual(116);
    expect(enriched.mods[0]).toEqual({ id: "architectury", name: "Architectury", version: "2.0.7", isSuspected: false });
    expect(enriched.mods[1]).toEqual({ id: "audiooutput", name: "AudioOutput", version: "0.0.4", isSuspected: false });
    //start: 141
    expect(enriched.mods[76]).toEqual({ id: "fabrishot", name: "Fabrishot", version: "1.5.0", isSuspected: false });
    expect(enriched.mods[108]).toEqual({ id: "seamless_loading_screen", name: "Seamless Loading Screen", version: "1.3.5+1.17", isSuspected: true });
    expect(enriched.mods[109]).toEqual({ id: "smoothboot", name: "Smooth Boot", version: "1.16.5-1.6.0", isSuspected: false });
    expect(enriched.mods[115]).toEqual({ id: "transliterationlib", name: "TRansliterationLib", version: "1.1.0", isSuspected: true });
    //	at org.spongepowered.asm.mixin.transformer.MixinProcessor.applyMixins(MixinProcessor.java:363)
    expect(enriched.stackTrace.elements[0]).toEqual({
        method: Mappable_1.JavaMethod.dotSeperated("org.spongepowered.asm.mixin.transformer.MixinProcessor", "applyMixins"),
    ...
    var cause = enriched.stackTrace.causedBy;
    expect(cause.title).toEqual({
        ...
        class: Mappable_1.JavaClass.dotSeperated("org.spongepowered.asm.mixin.throwables.MixinApplyError")
    });
...
exports.testFabricCrashReportEnrich = testFabricCrashReportEnrich;
function testForgeCrashReportEnrich(enriched) {
    ...
    expect(enriched.stackTrace.title).toEqual({
        ...
        class: Mappable_1.JavaClass.dotSeperated("java.lang.NullPointerException")
    });
    ...
    expect(enriched.stackTrace.elements[0]).toEqual({
        method: Mappable_1.JavaMethod.dotSeperated("net.minecraft.client.renderer.GameRenderer", "func_78473_a"),
    ...
    expect(enriched.stackTrace.elements[1]).toEqual({
        method: Mappable_1.JavaMethod.dotSeperated("net.minecraft.client.Minecraft", "func_71407_l"),
    ...
    expect(enriched.stackTrace.elements[6]).toEqual({
        method: Mappable_1.JavaMethod.dotSeperated("net.minecraft.client.main.Main", "main"),
    ...
    expect(enriched.stackTrace.elements[7]).toEqual({
        method: Mappable_1.JavaMethod.dotSeperated("sun.reflect.NativeMethodAccessorImpl", "invoke0"),
    ...
    expect(enriched.stackTrace.elements[11]).toEqual({
        method: Mappable_1.JavaMethod.dotSeperated("net.minecraftforge.fml.loading.FMLClientLaunchProvider", "lambda$launchService$0"),
    ...
    expect(enriched.stackTrace.elements[17]).toEqual({
        method: Mappable_1.JavaMethod.dotSeperated("cpw.mods.modlauncher.Launcher", "main"),
    ...
    var time = enriched.context.time;
    //15.08.21 17:36
    expect(time).toEqual(new Date(2021, 8, 15, 17, 36));
    expect(enriched.context.loader).toEqual({
...
test("Fabric crash report is enriched properly", function () {
    var enriched = (0, CrashReportEnricher_1.enrichCrashReport)((0, CrashReportParser_1.parseCrashReport)(TestCrashes_1.testFabricCrashReport));
    testFabricCrashReportEnrich(enriched);
});
test("Forge crash report is enriched properly", function () {
    var enriched = (0, CrashReportEnricher_1.enrichCrashReport)((0, CrashReportParser_1.parseCrashReport)(TestCrashes_1.testForgeCrashReport));
