...
exports.testFabricCrashReportParse = exports.testForgeCrashReportParse = void 0;
var CrashReportParser_1 = require("../crash/parser/CrashReportParser");
var TestCrashes_1 = require("./TestCrashes");
var TestBuggyParseCrash_1 = require("./TestBuggyParseCrash");
var CrashReportEnricher_1 = require("../crash/parser/CrashReportEnricher");
var BarebonesFabricCrash_1 = require("./BarebonesFabricCrash");
var RichCrashReport_1 = require("../crash/model/RichCrashReport");
var TestVerifyErrorCrash_1 = require("./TestVerifyErrorCrash");
var BrokenTimeCrash_1 = require("./BrokenTimeCrash");
var TestEmptySectionCrash_1 = require("./TestEmptySectionCrash");
var BrokenSectionCrash_1 = require("./BrokenSectionCrash");
function testForgeCrashReportParse(report) {
    ...
    var systemDetails = report.sections[1].details;
    expect(systemDetails["Minecraft Version"]).toEqual("1.16.5");
    expect(systemDetails["Minecraft Version ID"]).toEqual("1.16.5");
    expect(systemDetails["Operating System"]).toEqual("Windows 7 (x86) version 6.1");
    expect(systemDetails["Memory"]).toEqual("550771464 bytes (525 MB) / 1073741824 bytes (1024 MB) up to 1073741824 bytes (1024 MB)");
    expect(systemDetails["JVM Flags"]).toEqual("9 total; -XX:HeapDumpPath=MojangTricksIntelDriversForPerformance_javaw.exe_minecraft.exe.heapdump -Xss1M -Xmx1G -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=16M");
...
exports.testForgeCrashReportParse = testForgeCrashReportParse;
function testFabricCrashReportParse(report) {
    ...
    var childStackTrace = report.stacktrace.causedBy;
    expect(childStackTrace.message).toEqual("org.spongepowered.asm.mixin.throwables.MixinApplyError: Mixin [screenshotclipboard.mixins.json:ScreenshotMixin] from phase [DEFAULT] in config [screenshotclipboard.mixins.json] FAILED during APPLY");
    ...
    var lastReload = report.sections[2];
    ...
    var reloadDetails = lastReload.details;
    ...
    var systemDetails = report.sections[3].details;
    ...
    var optifabric = report.sections[4];
    expect(optifabric.title).toEqual("OptiFabric");
    var optifabricDetails = optifabric.details;
...
exports.testFabricCrashReportParse = testFabricCrashReportParse;
test('Forge Crash Report is parsed correctly', function () {
    var report = (0, CrashReportParser_1.parseCrashReport)(TestCrashes_1.testForgeCrashReport);
    testForgeCrashReportParse(report);
});
test("Fabric crash report is parsed correctly", function () {
    var report = (0, CrashReportParser_1.parseCrashReport)(TestCrashes_1.testFabricCrashReport);
    testFabricCrashReportParse(report);
});
test("Crash that was found to be badly parsed is parsed correctly", function () {
    var report = (0, CrashReportParser_1.parseCrashReport)(TestBuggyParseCrash_1.TestBuggyParseCrash);
    expect(report.sections.length).toEqual(5);
    var enriched = (0, CrashReportEnricher_1.enrichCrashReport)(report);
    expect(enriched.context.time).toEqual(new Date(2021, 10, 3, 12, 22));
});
test("Barebones Fabric crash is parsed correctly", function () {
    var report = (0, CrashReportParser_1.parseCrashReport)(BarebonesFabricCrash_1.BarebonesFabricCrash);
    var enriched = (0, CrashReportEnricher_1.enrichCrashReport)(report);
    expect(enriched.context.loader.type).toEqual(RichCrashReport_1.LoaderType.Fabric);
    expect(enriched.context.loader.version).toEqual(undefined);
    expect(enriched.mods).toEqual(undefined);
});
test("VerifyError crash is parsed correctly", function () {
    var _a, _b, _c;
    var report = (0, CrashReportParser_1.parseCrashReport)(TestVerifyErrorCrash_1.TestVerifyErrorCrash);
    expect(report.sections.length).toEqual(4);
    expect(report.stacktrace.trace.length).toEqual(11);
    expect(Object.values((_a = report.stacktrace.details) === null || _a === void 0 ? void 0 : _a.details).length).toEqual(5);
    expect((_b = report.stacktrace.details) === null || _b === void 0 ? void 0 : _b.details["Current Frame"].length).toEqual(4);
    expect((_c = report.stacktrace.details) === null || _c === void 0 ? void 0 : _c.details["Location"]).toEqual(["net/minecraft/class_5944.<init>(Lnet/minecraft/class_5912;Ljava/lang/String;Lnet/minecraft/class_293;)V @7: invokespecial"]);
    var enriched = (0, CrashReportEnricher_1.enrichCrashReport)(report);
    //2021-10-25, 10:07 a.m.
    expect(enriched.context.time).toEqual(new Date(2021, 10, 25, 10, 7));
    var details = enriched.stackTrace.details;
    ...
    var frame = details.currentFrame;
...
test("Time is parsed correctly in edge case", function () {
    var report = (0, CrashReportParser_1.parseCrashReport)(BrokenTimeCrash_1.BrokenTimeCrash);
    var enriched = (0, CrashReportEnricher_1.enrichCrashReport)(report);
...
test("Empty space crash is parsed correctly", function () {
    var report = (0, CrashReportParser_1.parseCrashReport)(TestEmptySectionCrash_1.TestEmptySectionCrash);
    expect(report.sections.length).toEqual(3);
});
test("Broken section crash is recognized as broken", function () {
    expect(function () {
        (0, CrashReportParser_1.parseCrashReportImpl)(BrokenSectionCrash_1.BrokenSectionCrash, true);
